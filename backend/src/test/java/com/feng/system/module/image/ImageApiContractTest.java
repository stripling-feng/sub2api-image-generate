...
class ImageApiContractTest {
    ...
    void preservesAllExpressRoutesAndRawResponseShapes() throws Exception {
        ...
        when(sessions.resolveProfile(any(), any())).thenReturn(null);
        ...
        mvc.perform(get("/api/health")).andExpect(status().isOk()).andExpect(jsonPath("$.ok").value(true));
        ...
        mvc.perform(post("/api/session/logout")).andExpect(status().isOk()).andExpect(jsonPath("$.ok").value(true));
        mvc.perform(get("/api/session/me")).andExpect(status().isOk()).andExpect(jsonPath("$.profile").doesNotExist());
